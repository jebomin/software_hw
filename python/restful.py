...
app = Flask(__name__)
cluster = MongoClient("mongodb://user_name:user_password@SERVER_IP/prod-db")
...
def get_all_document():
  ...
  for s in collection.find():
    s["_id"] = str(s["_id"]) # id를 string으로 바꿔줘야만 출력이가능하다
  output.append(
...
def add_document():
  ...
  document_id = collection.insert({'name': name, 'score': score})
